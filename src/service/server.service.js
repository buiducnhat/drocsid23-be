...
const ServerRoleGroupService = require('./serverRoleGroup.service');
const UserServerRoleService = require('./userServerRole.service');
...
const ServerService = {
    create: async (ownerId, name, description, isPublic) => {
        try {
            ...
            await UserServerRoleModel.create({
                userId: ownerId,
                serverId: newServer.id,
                serverRoleGroupId: [everyone.id, admin.id],
            });
    ...
    getByID: async (id, userId) => {
        try {
            ...
            const server = await ServerModel.findById(id).populate('members', 'email fullname avatarUrl');
            ...
            let roles = (await ServerRoleGroupService.getAll(id)).data;
            let rr = [];
    ...
    joinServer: async (userId, serverId) => {
        try {
            ...
            if (server.members.includes(userId))
                throw new Error(`Cant joined server: User: ${userId} was a member of server`);
    ...
    denyUserRequestJoin: async (userIdRequest, serverId) => {
        try {
            ...
            if (!server.requestJoinUsers.includes(userIdRequest))
                throw new Error(`User: ${userIdRequest} is not request to join server: ${serverId}`);
