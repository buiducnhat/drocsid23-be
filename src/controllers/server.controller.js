...
const ServerController = {
    ...
    responseUserRequestJoin: async (req, res, next) => {
        try {
            ...
            let response;
            if (acceptJoin) response = await ServerService.joinServer(userIdRequest, req.params.serverId);
            else response = await ServerService.denyUserRequestJoin(userIdRequest, req.params.serverId);
            if (response.status === ERR)
                throw new CusError(apiStatus.INVALID_PARAM, httpStatus.NOT_FOUND, response.data);
    ...
    createInviteServer: async (req, res, next) => {
        try {
            ...
            const { status, data } = await InviteService.createInvite(
                req.userId,
                req.body.expireTime,
                req.params.serverId,
                0,
